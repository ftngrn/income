<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $childParent->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $childParent->id)]
            )
        ?></li>
...
<div class="childParents form large-9 medium-8 columns content">
    <?= $this->Form->create($childParent) ?>
    <fieldset>
        <legend><?= __('Edit Child Parent') ?></legend>
        <?php
